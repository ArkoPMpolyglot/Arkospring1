...
function [pChg,pDis] = HPPCpower(z0,T,dT,eta,ns,np,model,limits)
  ...
  %computing the ocv for discharge
  ...
  %code for pDis and pChg begins
  pDis = min(ns*vMin*iDisMax,ns*pMax*ones(size(z0)));  % You will need to change this to compute it correctly
  
  
  pChg = min(0,max(ns*vMax*iChgMin,ns*pMin*ones(size(z0)))); % You will need to change this to compute it correctly
end
